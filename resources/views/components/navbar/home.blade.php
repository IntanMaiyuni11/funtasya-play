<nav class="bg-white border-b sticky top-0 z-50" x-data="{ open: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-center justify-between">
            ...
            {{-- Bagian Kanan: User Profile & Cart --}}
            <div class="flex items-center gap-2 md:gap-4">
                @auth
                    <div class="flex items-center gap-2 md:gap-3">
                        ...
                        {{-- Icon Cart dengan Counter E-commerce --}}
                        <a href="/cart" class="w-9 h-9 md:w-10 md:h-10 bg-[#EC4899] rounded-lg flex items-center justify-center text-white hover:bg-pink-600 transition-colors relative">
                            ...
                            @php
                                // Menghitung total quantity semua barang di cart untuk user yang login
        ...
        <div x-show="open" 
             ...
             class="md:hidden mt-4 pb-4 space-y-4 border-t pt-4">
            
            <form action="{{ route('catalog') }}" method="GET" class="relative">
                <input type="text" name="search" placeholder="Cari..." class="w-full pl-10 pr-4 py-2 rounded-full bg-gray-100 text-sm focus:outline-none">
            ...
            <ul class="space-y-3 font-medium text-gray-600 px-2">
                <li><a href="{{ route('home') }}" class="block hover:text-[#EC4899]">Home</a></li>
                <li><a href="{{ route('about') }}" class="block hover:text-[#EC4899]">About us</a></li>
                <li><a href="{{ route('catalog') }}" class="block hover:text-[#EC4899]">Catalog</a></li>
            </ul>
